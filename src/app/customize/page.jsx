...
export default function CustomizePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white text-slate-900 max-md:pb-24">
      {/* HERO */}
      <div className="mx-auto px-4">
        <Card className="p-6 rounded-2xl shadow-xl">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold text-emerald-800">Customize Your Cup</h1>
              <p className="mt-1 text-slate-600 max-sm:text-[14px]">Choose fruits, adjust quantity (grams), add toppings — price & nutrition update live.</p>
            </div>
      ...
      {/* MAIN CONTENT */}
      <main className="mx-auto px-4 py-6 space-y-8 max-w-[1200px]">
          ...
          <section className="md:col-span-2 space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                snap-x snap-mandatory scroll-smooth -mx-4 px-4">
                    {FRUITS.map((f) => (
                      <div
                        key={f.id}
                        className={`min-w-[220px] w-56 flex-shrink-0 rounded-xl border p-3
                  ${qtyByFruit[f.id] > 0 ? "border-emerald-600 bg-emerald-50" : "border-gray-200 bg-white"}
                  snap-start`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-slate-500">
                              {f.calories} kcal /100g • Vit C {f.vitaminC_mg}mg
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          <button
                            onClick={() =>
                              updateQty(f.id, Math.max(0, (qtyByFruit[f.id] || 0) - 25))
                            }
                          ...
                          <input
                            type="number"
                            value={qtyByFruit[f.id] || 0}
                            onChange={(e) => updateQty(f.id, Number(e.target.value || 0))}
                            className="w-20 text-center border rounded-md px-2 py-1"
                          />
                          <button
                            onClick={() => updateQty(f.id, (qtyByFruit[f.id] || 0) + 25)}
                            className="px-2 py-1 rounded-md bg-white border"
                          >
                            +
                          </button>
                          <div className="ml-auto text-sm font-semibold text-amber-600">
                            ₹{Math.round(((qtyByFruit[f.id] || 0) / 100) * f.pricePer100)}
                          </div>
                        </div>

                        <div className="mt-2 text-xs text-slate-500 flex items-center gap-2">
                          <Badge>
                            {Math.round(f.calories * ((qtyByFruit[f.id] || 0) / 100))} kcal
                          </Badge>
                          <span>
                            {Math.round(f.vitaminC_mg * ((qtyByFruit[f.id] || 0) / 100))} mg
                            Vit C
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {FRUITS.map((f) => (
                      <div
                        key={f.id}
                        className={`rounded-xl border p-3 ${qtyByFruit[f.id] > 0
                ...
                <div className="flex flex-wrap gap-3">
                  {TOPPINGS.map((t) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg border ${selectedToppings.includes(t.id)
            ...
            <Card className="bg-emerald-50 border-emerald-200">
              ...
              <CardContent className="space-y-3">
                <p className="text-sm text-slate-700">
                  Choose Normal Premium (we decide fruits) or Custom Premium (you choose). Members
                  get discounts & free delivery above ₹100.
                </p>

                <div className="grid sm:grid-cols-2 gap-3">
                  <select
                    value={daysPlan}
                    onChange={(e) => setDaysPlan(Number(e.target.value))}
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => setPremiumType("normal")}
                      className={`flex-1 rounded-md px-3 py-2 ${premiumType === "normal" ? "bg-amber-400 text-black" : "bg-white border"
                        }`}
                    >
                      Normal Premium
                    </button>
                    <button
                      onClick={() => setPremiumType("custom")}
                      className={`flex-1 rounded-md px-3 py-2 ${premiumType === "custom" ? "bg-amber-400 text-black" : "bg-white border"
                        }`}
                    >
                      Custom Premium
                    </button>
